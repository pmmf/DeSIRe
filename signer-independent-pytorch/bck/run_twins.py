...
from data.data import CelebA, KinectLeap, split_data, getSplitter
from models.twins import TWINS
...
from losses.losses import twins_loss, cross_entropy_loss
from utils.utils import merge_images, inverse_transform, annealing_function
...
def main():
    ...
    parser.add_argument('--output', default='/data/pmmf/junk/twins/')
    ...
    for split, (tr_indexes, test_indexes) in enumerate(dataSplitter):
        # print(split, (tr_indexes, test_indexes))
        # continue
        # ada
        # output fn
        ...
        # split data
        train_loader, valid_loader = split_data(dataset,
        ...
        cnn_val, acc_val = valid_eval(model, valid_loader, device)
        print('##!!!! Valid loss: {:.5f} |'.format(cnn_val.item()) +
                ' Valid Acc: {:.5f}'.format(acc_val))

        results.append((cnn_val.item(), acc_val))

    # save results
    print(results)

    res_fn = os.path.join(output_fn, 'res.pckl')
